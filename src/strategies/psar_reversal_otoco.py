...
class PsarReversalOtocoStrategy(BaseStrategy):
    def __init__(self, params: dict):
        ...
        required_params = ['psar_step', 'psar_max_step', 
                           'atr_period', 'sl_atr_mult', 'tp_atr_mult', 
                           'indicateur_frequence_psar', 'atr_base_frequency'] 
    ...
    def _calculate_indicators(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.psarl_col_strat not in df.columns:
            df[self.psarl_col_strat] = np.nan
        if self.psars_col_strat not in df.columns:
            df[self.psars_col_strat] = np.nan
    ...
    def generate_signals(self, data: pd.DataFrame):
        ...
        psarl_prev_filled = df_with_indicators[self.psarl_col_strat].shift(1).fillna(method='bfill').fillna(close_prev + 1e9) 
        psars_prev_filled = df_with_indicators[self.psars_col_strat].shift(1).fillna(method='bfill').fillna(close_prev - 1e9)
        ...
        long_condition = (close_prev <= psars_prev_filled) & (close_curr > psarl_curr) & psarl_curr.notna() & psars_prev_filled.notna() & close_prev.notna()
        short_condition = (close_prev >= psarl_prev_filled) & (close_curr < psars_curr) & psars_curr.notna() & psarl_prev_filled.notna() & close_prev.notna()
        
        signals_df = pd.DataFrame(index=df_with_indicators.index)
        signals_df['entry_long'] = long_condition
        signals_df['entry_short'] = short_condition
        signals_df['exit_long'] = False 
        signals_df['exit_short'] = False

        signals_df['sl'] = np.where(
            long_condition & atr_curr.notna(), close_curr - sl_atr_mult * atr_curr, 
            np.where(short_condition & atr_curr.notna(), close_curr + sl_atr_mult * atr_curr, np.nan)
        )
        signals_df['tp'] = np.where(
            long_condition & atr_curr.notna(), close_curr + tp_atr_mult * atr_curr, 
            np.where(short_condition & atr_curr.notna(), close_curr - tp_atr_mult * atr_curr, np.nan)
        )
    ...
    def generate_order_request(self,
        ...
        if pd.notna(psars_prev) and pd.notna(psarl_curr) and close_prev <= psars_prev and close_curr > psarl_curr: 
            side = 'BUY'
            stop_loss_price_raw = close_curr - sl_atr_mult * atr_value
            take_profit_price_raw = close_curr + tp_atr_mult * atr_value
        elif pd.notna(psarl_prev) and pd.notna(psars_curr) and close_prev >= psarl_prev and close_curr < psars_curr: 
            side = 'SELL'
            stop_loss_price_raw = close_curr + sl_atr_mult * atr_value
            take_profit_price_raw = close_curr - tp_atr_mult * atr_value
