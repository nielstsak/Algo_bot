...
class MaCrossoverStrategy(BaseStrategy):
    def __init__(self, params: dict):
        ...
        required_params = ['fast_ma_period', 'slow_ma_period', 'ma_type', 
                           'atr_period', 'sl_atr_multiplier', 'tp_atr_multiplier', 
                           'indicateur_frequence_ma_rapide', 'indicateur_frequence_ma_lente', 
                           'atr_base_frequency'] 
        missing = [p for p in required_params if self.get_param(p) is None]
        if missing:
    ...
    def _calculate_indicators(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.fast_ma_col_strat not in df.columns:
            df[self.fast_ma_col_strat] = np.nan
        if self.slow_ma_col_strat not in df.columns:
            df[self.slow_ma_col_strat] = np.nan
    ...
    def generate_signals(self, data: pd.DataFrame):
        ...
        buy_cond = (ma_fast_curr > ma_slow_curr) & (ma_fast_prev <= ma_slow_prev) & ma_fast_curr.notna() & ma_slow_curr.notna() & ma_fast_prev.notna() & ma_slow_prev.notna()
        sell_cond = (ma_fast_curr < ma_slow_curr) & (ma_fast_prev >= ma_slow_prev) & ma_fast_curr.notna() & ma_slow_curr.notna() & ma_fast_prev.notna() & ma_slow_prev.notna()

        signals_df = pd.DataFrame(index=df_with_indicators.index)
        signals_df['entry_long'] = buy_cond
        signals_df['entry_short'] = sell_cond
        signals_df['exit_long'] = False 
        signals_df['exit_short'] = False

        signals_df['sl'] = np.where(
            buy_cond & atr_curr.notna(), close_curr - sl_mult * atr_curr, 
            np.where(sell_cond & atr_curr.notna(), close_curr + sl_mult * atr_curr, np.nan)
        )
        signals_df['tp'] = np.where(
            buy_cond & atr_curr.notna(), close_curr + tp_mult * atr_curr, 
            np.where(sell_cond & atr_curr.notna(), close_curr - tp_mult * atr_curr, np.nan)
        )
    ...
    def generate_order_request(self,
        ...
        required_cols = ['close', self.fast_ma_col_strat, self.slow_ma_col_strat, self.atr_col_strat]
        if latest_data[required_cols].isnull().any() or \
           previous_data[['close', self.fast_ma_col_strat, self.slow_ma_col_strat]].isnull().any():
            return None
        ...
        sl_atr_mult = self.get_param('sl_atr_multiplier') 
        tp_atr_mult = self.get_param('tp_atr_multiplier') 
