...
class KamaCrossoverOtocoStrategy(BaseStrategy):
    def __init__(self, params: dict):
        super().__init__(params)
        required_params = ['kama_period', 'kama_fast_ema', 'kama_slow_ema', 
                           'atr_period', 'sl_atr_mult', 'tp_atr_mult',      
                           'indicateur_frequence_kama', 'atr_base_frequency'] 
    ...
    def _calculate_indicators(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.kama_col_strat not in df.columns:
            df[self.kama_col_strat] = np.nan
    ...
    def generate_signals(self, data: pd.DataFrame):
        ...
        required_cols_for_signal = [self.kama_col_strat, self.atr_col_strat, 'close', 'open', 'high', 'low']
        ...
        kama_prev = df_with_indicators[self.kama_col_strat].shift(1)
        kama_curr = df_with_indicators[self.kama_col_strat]
        atr_curr = df_with_indicators[self.atr_col_strat]

        long_condition = (close_prev <= kama_prev) & (close_curr > kama_curr) & kama_curr.notna() & close_prev.notna() & kama_prev.notna()
        short_condition = (close_prev >= kama_prev) & (close_curr < kama_curr) & kama_curr.notna() & close_prev.notna() & kama_prev.notna()
    ...
    def generate_order_request(self,
        ...
        required_cols = ['close', self.kama_col_strat, self.atr_col_strat]
        if latest_data[required_cols].isnull().any() or \
           previous_data[['close', self.kama_col_strat]].isnull().any():
            return None

        close_curr = latest_data['close']
        kama_curr = latest_data[self.kama_col_strat]
        close_prev = previous_data['close']
        kama_prev = previous_data[self.kama_col_strat]
